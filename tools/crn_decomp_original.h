...
namespace crnd
{
   ...
   {
      if (size > (1u<<20)) fprintf(stderr, "DBG: big alloc %zu\n", size);
   ...
   {
      if ((ptr_bits)reinterpret_cast<ptr_bits>(p) & (CRND_MIN_ALLOC_ALIGNMENT - 1))
      {
...
#define CRND_FOURCC(a, b, c, d) ((a) | ((b) << 8U) | ((c) << 16U) | ((d) << 24U))
   ...
   const crn_header* crnd_get_header(crn_header& tmp_header, const void* pData, uint32 data_size)
   {
      ...
      if ((!pData) || (data_size < sizeof(crn_header)))
         { fprintf(stderr, "DBG: get_header size fail ds=%zu\n", data_size); return NULL; }

      const crn_header& file_header = *static_cast<const crn_header*>(pData);
      if (file_header.m_sig != crn_header::cCRNSigValue)
         { fprintf(stderr, "DBG: get_header sig fail %04X\n", (unsigned)file_header.m_sig); return NULL; }

      if ((file_header.m_header_size < sizeof(crn_header)) || (data_size < file_header.m_data_size))
         { fprintf(stderr, "DBG: get_header hs=%u sizeof=%zu dsz=%u ds=%u\n", (unsigned)file_header.m_header_size, sizeof(crn_header), (unsigned)file_header.m_data_size, data_size); return NULL; }
...
namespace crnd
{
   ...
   private:
      ...
      bool init_tables()
      {
         if (!m_codec.start_decoding(m_pData + m_pHeader->m_tables_ofs, m_pHeader->m_tables_size))
            { fprintf(stderr, "DBG: start_decoding failed\n"); return false; }

         if (!m_codec.decode_receive_static_data_model(m_chunk_encoding_dm))
            { fprintf(stderr, "DBG: chunk_encoding_dm failed\n"); return false; }
         ...
         {
            if (!m_codec.decode_receive_static_data_model(m_endpoint_delta_dm[0])) { fprintf(stderr, "DBG: ep dm failed\n"); return false; }
            if (!m_codec.decode_receive_static_data_model(m_selector_delta_dm[0])) { fprintf(stderr, "DBG: sel dm failed\n"); return false; }
         }

         if (m_pHeader->m_alpha_endpoints.m_num)
         {
            if (!m_codec.decode_receive_static_data_model(m_endpoint_delta_dm[1])) { fprintf(stderr, "DBG: alpha ep dm failed\n"); return false; }
            if (!m_codec.decode_receive_static_data_model(m_selector_delta_dm[1])) { fprintf(stderr, "DBG: alpha sel dm failed\n"); return false; }
         }

         m_codec.stop_decoding();

         fprintf(stderr, "DBG: init_tables OK\n");
         return true;
      }
      ...
      bool decode_palettes()
      {
         ...
         if (m_pHeader->m_alpha_endpoints.m_num)
         {
            if (!decode_alpha_endpoints()) { fprintf(stderr, "DBG: alpha_endpoints failed\n"); return false; }
            if (!decode_alpha_selectors()) { fprintf(stderr, "DBG: alpha_selectors failed\n"); return false; }
         }

         fprintf(stderr, "DBG: palettes OK\n");
         return true;
      }
      ...
      bool decode_color_selectors()
      {
         ...
         if (!m_codec.start_decoding(m_pData + m_pHeader->m_color_selectors.m_ofs, m_pHeader->m_color_selectors.m_size))
            { fprintf(stderr, "DBG: decode_color_selectors fail line 1\n"); return false; }

         static_huffman_data_model dm;
         if (!m_codec.decode_receive_static_data_model(dm))
            { fprintf(stderr, "DBG: decode_color_selectors fail line 2\n"); return false; }
         ...
         if (!m_color_selectors.resize(num_color_selectors))
            { fprintf(stderr, "DBG: decode_color_selectors fail line 3\n"); return false; }
   ...
   crnd_unpack_context crnd_unpack_begin(const void* pData, uint32 data_size)
   {
      ...
      if (!p)
         { fprintf(stderr, "DBG: new unpacker NULL\n"); return NULL; }

      if (!p->init(pData, data_size))
      {
         fprintf(stderr, "DBG: init false\n");
         crnd_delete(p);
         return NULL;
      }
